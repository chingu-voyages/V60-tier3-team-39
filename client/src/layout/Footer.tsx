      ...
      <div className='relative px-4 sm:px-6 py-8 md:px-8 lg:p-10 max-w-7xl mx-auto space-y-6'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3'>
          {team.map(({ name, role, linkedin, github }) => (
            <div
              key={name}
              className='flex flex-col justify-between border border-muted/20 p-3 lg:p-4 rounded-[10px] space-y-4 hover:border-brand hover:bg-brand/5 transition-colors duration-400'>
              <div>
                <h2 className='text-sm lg:text-base font-semibold text-primary'>{name}</h2>
